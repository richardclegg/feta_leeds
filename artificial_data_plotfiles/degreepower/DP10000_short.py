...
fig, ax = plt.subplots(nrows=1,ncols=1,figsize=(6,3.5),sharex=True)
...
ax.set_ylabel('Estimated changepoint time', fontsize=16)
#plt.ylim([0,300])
...
for comb in params:
    ...
    for ex in range(experiments):
        file = "degreepower"+comb[0]+"-"+comb[1]+"-10000results"+str(ex+1)+".dat"
        lines=[]
    ...
    means = np.array([sum(row)/experiments for row in MLEtable])[:9]
    sds = np.array([math.sqrt(sum((MLEtable[i,:] - means[i])**2)/experiments) for i in range(9)])
    ...
    if comb == ['1.0', '0.9'] or comb == ['1.2', '1.0']:
        ax.fill_between(truevals,means-symerror,means+symerror,alpha=fillalphas[str(comb)], color=colourz[str(comb)], label=comb[0]+" to "+comb[1])
        ax.plot(truevals,means, color=colourz[str(comb)])



ax.plot(truevals,truevals, linestyle='--', color='black')
ax.legend(loc='upper left')
plt.tight_layout()
#plt.show()

fig.savefig("DP10000_diff_markers.pdf")
